...
class GPT(nn.Module):
    ...
    def forward(self, idx,targets=None):
        ...
        pos_emb=self.transformer.wpe(pos)#shape(B,T,n_embd)
        tok_emb=self.transformer.wte(idx)#shape(B,T,n_embd)
        x=tok_emb+pos_emb
        #forward the blocks of the transformers
        for block in self.transformer.h:
            x=block(x)
        #forward the final layer norm and lm head
...
for step in range(max_steps):
    ...
    print(f"Step {step}: Loss = {loss.item()},lr = {lr} , norm: {norm:.4f} , dt: {dt:.2f}ms", f"tokens/sec: {tokens_per_sec:.2f}")
